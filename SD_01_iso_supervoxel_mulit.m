...
% VI_metric = 'DIR_HU';
VI_metric = 'DIR_Jac';
% VI_metric = 'Supervoxel_NO_Linear';
...
for num = 12 :subject_num
    ...
    Tptv_new = Tptv;
    for iter = 1:5
        [Tptv_new] = Calculate_Jac(vol_ex_ROI, Tptv_new, spc);
    end
    if strcmp(VI_metric,'Supervoxel_NO_Linear')
    ...
    for multi_layer = 1:5
        ...
        %         [V_before,V_after,N_in_after,C_p_in] = Supervoxel_inhale(SLIC_Labels_3D,Tptv_new,spc,labels_index);
        [V_before,V_after,N_in_after,C_p_in] = Supervoxel_in(SLIC_Labels_3D,Tptv_new,spc,labels_index);
        %         [V_after,C_p_in] = Supervoxel_in_forward_backward(SLIC_Labels_3D,Tptv,spc,labels_index);
        if strcmp(VI_metric,'DIR_Jac')
            [vent_img_cur] = Ventilation_Jac_iso(vol_ex_ROI, Tptv, spc);
        else
                ...
                if strcmp(VI_metric,'Supervoxel')
                    ...
                    if strcmp(VI_metric,'Supervoxel_NO_Linear')
                        [vent_img_cur] = Ventilation_Supervoxel_no_linear_19(vol_ex_nonor,vol_in_nonor,vent_jac_interp,SLIC_Labels_3D,...
                            V_after, C_p_in,labels_index,Tptv_new,spc);
                    end
...
%     GT_img_filter = GT_img;
    ...
    figure
    show_overlay_sagittal_CTVI_HU(avg_img_orig, vent_img_avg_filter, info_GT, GT_mask, index_slice,'hot')
