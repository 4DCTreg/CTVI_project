function vent_img = Ventilation_Supervoxel_v2(vol_ex,vol_in,vent_jac_interp,SLIC_Labels_3D,V_after,...
...
for id = 1:Numlabels
    ...
    if isempty(volin_sup)||isempty(C_p_ex_cur)
        ...
        volex_mass = 1000+vol_ex_sup(vol_ex_index);
        volex_air_sum = sum(volex_air(:));
        
        volin_mass = ((1000+vol_in_sup(volin_index))).*volin_index_mask;
        f_mass = (sum(volin_mass(:))-sum(volex_mass(:)))/sum(volex_mass(:));
        vol_in_mass_correction = (vol_in_sup(volin_index)-1000*f_mass*(1+vol_in_sup(volin_index)/1000)); 
        volin_air = ((-vol_in_mass_correction)).*volin_index_mask;
        volin_air_sum = sum(volin_air(:));
        sup_vent = volin_air_sum - volex_air_sum;
        
        
        %     nor_sup = round(abs(sup_vent)/n_voxel);
        %     sup_vent = sup_vent/nor_sup;
