...
ex_to_avg_path = 'H:/CTVI/Results/SD_01_ex_to_avg_111/';
in_to_ex_path = 'H:/CTVI/Results/SD_01_in_to_ex_iso_111/';
...
for num = 1 :subject_num
    ...
    crop_v_GT = crop_mask(GT_mask,size(GT_img),d);
    ...
    n_seeds = 2000;
    distance = round(nthroot(prod(sz_ct)/n_seeds,3));
    [SLIC_Labels_3D,Numlabels] = superpixels3(vol_ex,n_seeds);
    %     show_supervoxel_seg(vol_ex,SLIC_Labels_3D);
    index = 900;
    [V_before,C_p_ex,supvoxel_sz] = Supervoxel_Volume_single_layer(SLIC_Labels_3D,Numlabels);
    ...
    [V_after,C_p_in] = Supervoxel_in(SLIC_Labels_3D,Tptv,spc,Numlabels);
    if strcmp(VI_metric,'DIR_Jac')
        [vent_img] = Ventilation_Jac_iso(vol_ex, Tptv, spc);
    else
        if strcmp(VI_metric,'DIR_HU')
            [vent_img] = Ventilation_HU(vol_ex_nonor,vol_in_nonor, Tptv, spc);
        else
            if strcmp(VI_metric,'Supervoxel')
                [vent_img] = Ventilation_Supervoxel(vol_ex_nonor,vol_in_nonor,SLIC_Labels_3D,V_before,V_after, C_p_in,C_p_ex,supvoxel_sz,Numlabels,Tptv,spc);
            end
        end
    end
