...
%     parameter = homrf_get_POPI_parameter_multi_level(useTop,spc);
          
    [Knots_n_tot, Knots_n] = reg_homrf_multi_level(parameter,vol_in,vol_ex,...
            init_size,pts_ex,pts_in,bszv,crop_v); 
...
%     opts.metric = 'sad';
    ...
    vent_img_avg = ptv_deform(vent_img, T_pix, 0);
    if resize
        Tptv_rsz = cat(4, volresize(Tptv_co(:,:,:,1), bszv), volresize(Tptv_co(:,:,:,2), bszv), volresize(Tptv_co(:,:,:,3), bszv)); 
...
%     vent_img_avg = vent_img_avg./(-avg_img_HU_rsz/1000);
    ...
    % filter
    vent_img_avg = imgaussfilt3(vent_img_avg,0.5,'FilterSize',[5,5,3]);
    GT_img = imgaussfilt3(GT_img, 0.5, 'FilterSize', [5,5,3]);
...
save HOMRF_results.mat TREs_or TREs SpR R_tre_sp
