function vent_img = Ventilation_Supervoxel_no_linear_19(vol_ex,vol_in,vent_jac_interp,SLIC_Labels_3D,V_after,...
...
ex_all = [1:prod(im_sz)];
ex_parenchy = setdiff(ex_all,ex_mask_all);
vol_ex_sup = vol_ex;
vol_ex_sup(ex_mask_1) = -250;
vol_ex_sup(ex_mask_2) = -1000;
in_mask_1 = find(vol_in>-250);
in_mask_2 = find(vol_in<=-1000);
in_mask_all = [in_mask_1;in_mask_2];
in_parenchy = setdiff(ex_all,in_mask_all);
vol_in_sup = vol_in;
vol_in_sup(in_mask_1) = -250;
vol_in_sup(in_mask_2) = -1000;

% f_c = (sum(1000+vol_in_sup(in_parenchy))-sum(1000+vol_ex_sup(ex_parenchy)))/sum(1000+vol_ex_sup(ex_parenchy));
%         volin_sup_cor = vol_in_sup(in_parenchy)-1000*f_c*(1+vol_in_sup(in_parenchy)./1000);
%         vol_in_sup(in_parenchy) = volin_sup_cor;
...
for id = 1:Numlabels
    ...
    if isempty(volin_sup)||isempty(C_p_ex_cur)
        ...
        volex_air_mean = mean(volex_air(:));
        volex_air_sum = sum(-volex_air(:));
        volin_air = ((vol_in_sup(volin_index))).*volin_index_mask;
% volin_air = ((vol_in_sup(volin_sup)));
        volin_air_mean = mean(volin_air(:));
        volin_air_sum = sum(-volin_air(:));
        

% f_c = abs(((1000+volin_air_mean)-(1000+volex_air_mean))/(1000+volex_air_mean));
% volin_air_cor_mean = volin_air_mean-1000*f_c*(1+volin_air_mean./1000);
        
        
%         sup_vent = (  volex_air_mean-volin_air_cor_mean)/(volin_air_cor_mean+1000)*numel(volex_air);
        sup_vent = (  volex_air_mean-volin_air_mean)/(volin_air_mean+1000)*numel(volex_air);
%                 sup_vent = (  volex_air_mean-volin_air_mean)/(volin_air_mean+1000);
%         sup_vent = (  volex_air_mean-volin_air_mean);
%         sup_vent = (volin_air_sum - volex_air_sum)/volex_air_sum;
        
%         sup_vent = (
%         volex_air_mean-volin_air_mean)/(volin_air_mean+1000)*numel(volex_air);%%
%         not good
        ...
        if ceil(sup_coef_min)==ceil(sup_coef_max)
            ...
            if ceil(max_v) == ceil(min_v)
                sup_jac_coef = ones(size(sup_jac_coef,1),1);
                                ...
                                sup_jac_coef = img_thr_v1(sup_jac_coef, min(sup_jac_coef), max(sup_jac_coef), 1);
%                 sup_jac_coef = ones(size(sup_jac_coef,1),1);
            end
        end
...
%         x_mean = sup_vent/numel(vol_sup_ex_index);
