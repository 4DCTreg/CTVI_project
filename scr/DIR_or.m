function [pt_errs_phys,  TRE_phys, TREstd_phys] = DIR_or(pts_mov_pix, ...
                        pts_fix_pix, spc, lu_orig)
...
%     pts_moved_pix = round(pts_moved_pix);
    koef = repmat(spc, [size(pts_mov_pix, 1), 1]);
    pt_errs_phys = sqrt( sum((  (pts_mov_pix - pts_fix_pix).*koef  ).^2, 2) );
    TRE_phys = mean(pt_errs_phys);
    TREstd_phys = std(sqrt( sum((  (pts_mov_pix - pts_fix_pix).*koef  ).^2, 2) ));
end