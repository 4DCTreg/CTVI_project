function vent_img = Ventilation_Supervoxel_HU_v2(vol_ex,vol_in,SLIC_Labels_3D,...
    labels_index,Tptv_rsz,spc)
% this version use no classic HU method, single vol_in to single vol_ex;
...
for id = 1:Numlabels
    ...
    [c_x,c_y,c_z] = ind2sub(im_sz,vol_ex_index);
    Tx_cur = Tx(vol_ex_index);
    Ty_cur = Ty(vol_ex_index);
    Tz_cur = Tz(vol_ex_index);
    new_x = round(c_x + Tx_cur);
    new_y = round(c_y + Ty_cur);
    new_z = round(c_z + Tz_cur);
    new_x(find(new_x<1)) = 1;
    new_x(find(new_x>im_sz(1))) = im_sz(1);
    new_y(find(new_y<1)) = 1;
    new_y(find(new_y>im_sz(2))) = im_sz(2);
    new_z(find(new_z<1)) = 1;
    new_z(find(new_z>im_sz(3))) = im_sz(3);
    area_in_index = sub2ind(im_sz,new_x,new_y,new_z);
    sup_num = size(vol_ex_index,1);
    vol_ex_HU_sup = vol_ex_sup(vol_ex_index);
    vol_ex_avg_cur = mean(vol_ex_HU_sup);
    vol_in_HU_sup = vol_in_sup(area_in_index);
