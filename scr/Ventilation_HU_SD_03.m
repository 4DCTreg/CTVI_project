function [vent_img_mask] = Ventilation_HU_SD_03(vol_ex, vol_in, T_rsz, spc)
...
denominator = (vol_ex_mask.*(1000+vol_avg_mask));
denominator(find(denominator ==0)) = 1;
vent_img = 1000*(vol_avg_mask-vol_ex_mask)./denominator;
vent_img_mask = vent_img;
vent_index = reshape(vent_img_mask,[vol_size(1)*vol_size(2)*vol_size(3),1]);
vent_index_sort = sort(vent_index);
min_p_value = vent_index_sort(round(0.01*vol_size(1)*vol_size(2)*vol_size(3)));
max_p_value = vent_index_sort(round(0.99*vol_size(1)*vol_size(2)*vol_size(3)));
