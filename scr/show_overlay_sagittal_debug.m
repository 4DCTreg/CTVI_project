function show_overlay_sagittal_debug(img_CT, img_GT, info_GT, mask, index_slice,colormap_name)
...
p_value = round(img_GT_sort(round(0.05*new_sz(1)*new_sz(2)*new_sz(3))));
img_CT = img_thr(img_CT, -1024, 0, 1);
max_value_GT = max(img_GT(:));
min_value_GT = min(img_GT(:));
img_GT = img_thr(img_GT, p_value, max_value_GT, 1);
