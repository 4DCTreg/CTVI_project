function [vent_img] = Ventilation_Jac(vol_ex, Tptv_rsz, spc)
im_sz = size(vol_ex);
...
parfor index = 1:im_sz(1)*im_sz(2)*im_sz(3)
    ...
    vent_img(index) = mean((Jaco_1));
    
    
end
% vent_img(find(vent_img)<0) = 0;
min_value = min(vent_img(:));
max_value = max(vent_img(:));
vent_img(find(vent_img==0)) = min_value;
vent_img = img_thr(vent_img, min_value, max_value, 1500);
