...
class _3DGAN(object):
    ...
    def train(self):
        ...
        for step in range(self.start_step, 1 + self.config.max_iter):
            ...
            inputs, real_X = next(self.dataset.gen(True))
            inputs = [d.to('gpu').float().permute(0, 4, 1, 2, 3) for d in inputs]
            real_X = [d.to('gpu').float().permute(0, 4, 1, 2, 3) for d in real_X]
