...
for id = 1:Numlabels
    ...
    if isempty(volin_sup)||isempty(C_p_ex_cur)
        ...
        sup_vent = (volin_air_sum - volex_air_sum)/sum(volex_air_sum)*size(vol_ex_index,1);
        ...
        if ceil(sup_coef_min)==ceil(sup_coef_max)
            ...
            min_v = sup_vent_index_sort(round(0.02*numel(sup_jac_coef)));
            max_v = sup_vent_index_sort(round(0.98*numel(sup_jac_coef)));
