...
pts_all_phase = zeros(100,3,9);
koef = repmat(spc,100,1);
LMpath = 'H:/POPI_model/';
infpath = [LMpath,'case',num2str(idx),'/mhd/00.mhd'];
offset = [-250 -250 -164.5];
offset_all = repmat(offset,100,1);

for phase = 1:9
    LMname = [LMpath,'case',num2str(idx),'/case',num2str(idx),'_T',num2str(phase),'0.txt'];

    cur_pts = read_POPI_points_file(LMname)-offset_all;
...
for phase = 1:9
    spc = [0.976562 0.976562 2];
    filepath = 'H:/POPI_model/';
    filename = [filepath,'case',num2str(idx),'/mhd/',num2str(phase),'0.raw'];
    volmov = readrawPOPImeta(filename);
    filenamefix = [filepath,'case',num2str(idx),'/mhd/10.raw'];
    volfix=readrawPOPImeta(filenamefix);
    LMpath = 'H:/POPI_model/';
    LMname = [LMpath,'case',num2str(idx),'/case',num2str(idx),'_T',num2str(phase),'0.txt'];
    LMnamefix = [LMpath,'case',num2str(idx),'/case',num2str(idx),'_T10.txt'];
    pts_fix_or = read_POPI_points_file(LMnamefix)-offset_all;
    pts_mov_or = read_POPI_points_file(LMname)-offset_all;
    TIME_e = zeros(2,2,2, 10); 
    savepath = 'H:/groupwise_registration/POPI_iso_continus_all/';
    filenamesave = [savepath,'case',num2str(idx),'/case',num2str(phase),'_0.mat'];
