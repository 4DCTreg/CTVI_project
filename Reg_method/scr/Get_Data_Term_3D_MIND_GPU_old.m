...
division_fix = mat2cell(Fix_crop(:,:,:,1).^2,division_x,division_y,division_z);
...
testMov_img = padarray(Mov_crop,[add_hx,add_hy,add_hz,0]);


% cur = imfilter(testMov_img(:,:,:,1),Fix_crop(:,:,:,1));
...
gTarSearch = gpuArray(reshape(tarSearch,(blockSize_x+searchSize_x-1),(blockSize_y+searchSize_y-1),blockSize_z+searchSize_z-1,mindDim,Npoints));
