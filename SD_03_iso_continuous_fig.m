...
if strcmp(RefVI_type,'Galligas')
    ...
    if strcmp(RefVI_type,'Xenon')
        ...
        if strcmp(RefVI_type,'DTPA-SPECT')
            ...
            CTVI_path = 'H:/CTVI/Results/Continuous_figure/SD03/';
            subject_num = 21;
            num_phase = 10;
        end
    end
...
in_to_ex_path = 'H:/CTVI/Results/Continuous_figure/SD03/';
ex_to_avg_path = 'H:/CTVI/Results/SD_03_ex_to_avg_sup_multi/';
for num = 7:subject_num
    ...
    for phase_idx = 1:num_phase
        CTVI_name = [CTVI_path,'iso',VI_metric,'_Subject_0',num2str(num),num2str(phase_idx),'.mat'];
        if phase_idx == 6
            continue
        else
...
%     labels_index,Tptv,spc);
            [vent_img_cur] = Ventilation_Supervoxel_SD_03_nonormalize(vol_ex_nonor,vol_in_nonor,SLIC_Labels_3D,vent_HU,V_after,...
                C_p_in,labels_index,Tptv,spc);
%                         [vent_img_cur] = Ventilation_Supervoxel_SD_03_v1(vol_ex_nonor,vol_in_nonor,SLIC_Labels_3D,vent_HU,...
%                labels_index,Tptv,spc);
%             vent_img_cur = Ventilation_Supervoxel_v1(vol_ex_nonor,vol_in_nonor,vent_jac_interp,SLIC_Labels_3D,V_after,...
%     C_p_in,labels_index,Tptv,spc);
            %             [vent_img_cur] = Ventilation_Supervoxel_HU(vol_ex_nonor,vol_in_nonor,SLIC_Labels_3D,...
            %                 labels_index,Tptv,spc);
...
%     GT_img_filter = medfilt3(GT_img,[3,3,3]);
    ...
    index_slice = 256;
     ...
     show_overlay_sagittal_CTVI_SD_03_continuous(avg_img_orig, vent_img_avg_filter_all, info_GT, GT_mask, index_slice,'hot',num_phase-1)
