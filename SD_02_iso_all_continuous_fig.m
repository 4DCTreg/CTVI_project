...
if strcmp(RefVI_type,'Galligas')
    ...
    if strcmp(RefVI_type,'Xenon')
        ...
        CTVI_path_figure = 'H:/CTVI/Results/Continuous_figure/SD02/';
        subject_num = 4;
        num_phase = 8;
    else
...
in_to_ex_path = 'H:/CTVI/Results/Continuous_figure/SD02/';
for num = 3:subject_num
    ...
    vent_img_avg_filter_all =[];
    for phase_idx = 1:num_phase
        if phase_idx == 8
            continue
        else
        CTVI_name = [CTVI_path_figure,'iso',VI_metric,'_Subject_0',num2str(num),num2str(phase_idx),'to_ex','.mat'];
        vol_in_orig = double(vol_ct(:,:,:,phase_idx));
        vol_ex_orig = double(vol_ct(:,:,:,8));
        vol_in_mask = double(vol_mask(:,:,:,phase_idx));
        ...
        in_to_ex_name = [in_to_ex_path,'subject_',num2str(num),num2str(phase_idx),'.mat'];
        load(in_to_ex_name);
        if strcmp(VI_metric,'Supervoxel')
            ...
            for multi_layer = 1:5
                ...
                vol_ex_mask_crop = crop_data(vol_ex_mask,sup_crop_v_cur_recover);
                ...
                %             [vent_img_cur] = Ventilation_Supervoxel_SD02(vol_ex_nonor,vol_in_nonor,vol_ex_mask_crop,vent_jac_interp,SLIC_Labels_3D,...
                %                 V_after, C_p_in,labels_index,Tptv,spc);
                [vent_img_cur,vent_img_sup_cur] = Ventilation_Supervoxel_all_nonormalize(vol_ex_nonor,vol_in_nonor,vent_jac_interp,SLIC_Labels_3D,...
                    V_after, C_p_in,labels_index,Tptv,spc);
        ...
        vent_img_avg_filter_all = cat(4,vent_img_avg_filter_all,vent_img_avg_filter);
        end
    ...
    show_overlay_transection_CTVI_continuous(avg_img_orig, vent_img_avg_filter_all, info_GT, GT_mask, index_slice,'hot',num_phase-1)
